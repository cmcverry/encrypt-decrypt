...
int main(int argc, char* argv[]) {
    ...
    // Loops and extracts lines from plaintext file while not EOF
    ...
    // Checks that key is not smaller than plaintext
    ...
    // Sends "enc client" to server; used by server to verify compatible connection
    ...
    // Sends plaintextLength to server
    ...
    // Receive encrypted plaintext from server
    countChars = 0;
    ...
    // Outputs encrypted plaintext with new line character at end to stdout 
    printf("%s\n", bufferEncText);
